...
class Trainer(object):
    ...
    def __init__(self, config: SelfTrainConfig) -> None:
        ...
        # Setup paths
        self.data_cache_path = os.path.join(self.config.cache_dir,'v'+str(self.version),self.config.main_dataset)
        ...
        for idx, dataset in self.datasets.items():
            dataset_info = json.load(open(os.path.join(self.config.data_dir,dataset, TOKENIZED_DATA+'/train/dataset_info.json'),'r'))
    ...
    def _make_sets(self):
        ...
        while os.path.isfile(labeled_lock): 
            self.logger.debug('Waiting for other process to finish Labeled Set processing')
            sleep(60)
        ...
        # Check if unlabeled setup is running
        while os.path.isfile(unlabeled_lock): 
            self.logger.debug('Waiting for other process to finish Unabeled Set processing')
            sleep(60)
        ...
        while os.path.isfile(eval_lock): 
            self.logger.debug('Waiting for other process to finish Eval Set processing')
            sleep(60)
    ...
    def load_datsets(self):
        ...
        try:
            ...
            self.train_data = load_from_disk(os.path.join(self.config.data_dir,self.config.main_dataset,TOKENIZED_DATA+'/train'))
        ...
        try:
            ...
            self.test_data = load_from_disk(os.path.join(self.config.data_dir,self.config.main_dataset,TOKENIZED_DATA+'/test'))
        ...
        try:
            ...
            for dataset_idx, dataset_name in enumerate(self.config.other_datasets,1):
                ...
                dataset = load_from_disk(os.path.join(self.config.data_dir,dataset_name,TOKENIZED_DATA+'/train'))
...
if __name__ == "__main__":
    ...
    if args.run_name == None:
        trainer_config.run_name = 'test'
        trainer_config.strategy = 'pc_top_k'
        trainer_config.strategy_value = 100
        trainer_config.main_dataset = 'yahoo_answers'
        trainer_config.other_datasets = ['ag_news', 'dbpedia_14', 'yelp_review_full']
    
        trainer_config.labeled_size = 50000
        trainer_config.labeled_sampling = 'equal'
        trainer_config.unlabeled_size = 5000
        trainer_config.unlabeled_in_domain_ratio = 0.5
        trainer_config.main_sampling_ratio = 0.3
        trainer_config.max_batch_size = 32
