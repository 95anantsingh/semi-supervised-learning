...
function refresh_data(){
    dataset = dataset_selector.options[dataset_selector.selectedIndex].value;
    strategy = strategy_selector.options[strategy_selector.selectedIndex].value;
    strat_val = strat_val_selector.options[strat_val_selector.selectedIndex].value;
    sampling = sampling_selector.options[sampling_selector.selectedIndex].value;
    labeled = labeled_selector.options[labeled_selector.selectedIndex].value.split("_");
    unlabeled = unlabeled_selector.options[unlabeled_selector.selectedIndex].value.split("_");
    labeled_classes = labeled[0];
    labeled_size = labeled[1];
    unlabeled_classes = unlabeled[0];
    unlabeled_size = unlabeled[1];
    update_images();
}

function update_images(){
    img_dir="visuals/v2.0/"+dataset+"/"+sampling+strategy+"/"+strat_val+"_LC_"+labeled_classes+"_UC_"+unlabeled_classes+"_LS_"+labeled_size+"_US_"+unlabeled_size;
